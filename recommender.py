...
def pick_tiered_three(candidates, user_state, feedback):
    penalties, bonuses = _feedback_bias(feedback, user_state)
    ...
    popular = _pick_best(
        available,
        lambda movie: popular_score(movie)
        + _bonus_score(movie, bonuses)
...
def score_candidates(candidates, user_state, feedback):
    ...
    for movie in candidates:
        ...
        score += _bonus_score(movie, bonuses)
    ...
    for entry in feedback[-30:]:
        ...
        for genre_id in entry.get("genre_ids", []):
            if result == "no":
                ...
                bonuses[genre_id] = bonuses.get(genre_id, 0) + 0.12
...
def _bonus_score(movie, bonuses):
    return sum(bonuses.get(genre_id, 0) for genre_id in movie.get("genre_ids", []))
