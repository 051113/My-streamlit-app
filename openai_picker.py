...
def pick_movies_with_source(candidates, user_state, openai_api_key, feedback):
    if not candidates:
        return [], {}, None, "none"
    if not openai_api_key:
        picks = recommender.pick_top_three(candidates, user_state, feedback)
        reasons = recommender.template_reasons(picks, user_state)
        return picks, reasons, picks[0]["id"], "heuristic"
    try:
        response = _call_openai(candidates, user_state, openai_api_key)
        selected_ids, reasons = _validate_response(response, candidates)
        picks = [movie for movie in candidates if movie["id"] in selected_ids]
        ...
        if not recommender.is_tiered_order(picks):
            raise ValueError("Tier ordering invalid")
        return picks, reasons, selected_ids[0], "openai"
    except (ValueError, RuntimeError, json.JSONDecodeError):
        picks = recommender.pick_top_three(candidates, user_state, feedback)
        reasons = recommender.template_reasons(picks, user_state)
        return picks, reasons, picks[0]["id"], "heuristic"
...
def _call_openai(candidates, user_state, openai_api_key):
    ...
    reason_language = "Korean" if language == "ko-KR" else "English"
    system_message = (
        ...
        f"Write reasons in {reason_language}. Respond ONLY with JSON."
    )
    user_message = {
